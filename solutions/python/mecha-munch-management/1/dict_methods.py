...
def send_to_store(cart, aisle_mapping):
    ...
    for key in cart:
       if key in aisle_mapping:
           full_dict[key] = [cart[key]] + aisle_mapping[key]

    return dict(sorted(full_dict.items(), reverse=True))
...
def update_store_inventory(fulfillment_cart, store_inventory):
    ...
    for item in fulfillment_cart:
        if item in store_inventory:
            qty, aisle, fridge = store_inventory[item]
            new_qty = qty - fulfillment_cart[item][0]
            store_inventory[item][0] = new_qty if new_qty > 0 else "Out of Stock"
